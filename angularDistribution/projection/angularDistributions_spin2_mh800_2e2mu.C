...
void angularDistributions_spin2_mh800_2e2mu(int plotIndex=1, int isqq=0, int is2mp=0,TString decay="all", int binning=40 ){
...
*/
  ...
  RooRealVar* mzz = new RooRealVar("mzz","mzz",800,100,1000);
...
//  TChain* treeGrav = new TChain("candTree");
  treeGrav->Add("../../data/ggH_2bp_zz4l_M800_Ga247_all.root");
...
plot->Draw();
  ...
  sprintf(temp,"/afs/cern.ch/user/c/cayou/www/HighMass/test/%s_mH800_%s_%s.eps",measureables[plotIndex]->GetName(),linkname.Data(),decay.Data());
  can->SaveAs(temp);
  sprintf(temp,"/afs/cern.ch/user/c/cayou/www/HighMass/test/%s_mH800_%s_%s.png",measureables[plotIndex]->GetName(),linkname.Data(),decay.Data());
  can->SaveAs(temp);
