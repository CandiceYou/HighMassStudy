...
void eff_2l2q_spin2(int prod=0){
...
// read sample files
  ...
  TString  inputDir = "/afs/cern.ch/user/r/rbarr/Analysis/2l2q/CMSSW_8_0_7_wqin/src/ZZAnalysis/AnalysisStep/test/prod/2016_2l2q_spin2/";
  char inputfile[PATH_MAX];
  vector<TString> files_ggH,files_VBF;
  sprintf(inputfile,"output.root");
...
// initialize histograms
 ...
 const char* tagName[2]={"b-tagged","untagged"};
...
//vbf-tagging && b-tagging
        if ((typ==1 && btag1stJet>0.46 && btag2ndJet>0.46)||(typ==0 && btag1stSubjet>0.46 && btag2ndSubjet>0.46)) tag=0; //b-tagged
        else tag=1;  //untagged
...
//Fitting Function  par[0]*TMath::Erf( (x-par[1])/par[2] ) + (par[3] + par[4]*TMath::ATan((x-par[5])/par[6]));
  ...
  polyFunctot_resolved->SetParameters(0.248072, -1.715472, 2.500000, 177.456421, 401.930847, 0.005603, 1018.302307, 212.241714, 0.026830, 274.969147, 514.503784); polyFunctot_resolved->SetParameter(11, 300.406586);
...
//plotting
 ...
 TFile f (Form("2l2q_Efficiency_%s.root",prodName[prod]),"recreate");
 f.cd();
 for (int j=0;j<2;j++){
 ...
 //save parameters to file
         const char * param_txt = prod?Form("/afs/cern.ch/user/r/rbarr/www/HighMassStudy/16.7.19/VBF_%s_%s_%s.txt",lepName[j],jetName[i],tagName[k]):Form("/afs/cern.ch/user/r/rbarr/www/HighMassStudy/16.7.19/ggH_%s_%s_%s.txt",lepName[j],jetName[i],tagName[k]);
         ...
         fprintf(fp,"%s", "polyFunctot->SetParameters(");
          for (int i=0;i<polyFunctot->GetNpar();i++) {
              Float_t value = polyFunctot->GetParameter(i);
              if(i==11) fprintf(fp, ");\npolyFunctot->SetParameter(11, %f", value);
              else fprintf(fp,"%s%f", (i==0)?"":", ", value);
           }
           fprintf(fp, ");\n");
   ...
   c->Update();
   if (prod==0){ 
    c->SaveAs(Form("~/www/HighMassStudy/16.7.19/eff_ggH_all_%s.png",lepName[j]));
    c->SaveAs(Form("~/www/HighMassStudy/16.7.19/eff_ggH_all_%s.pdf",lepName[j]));}
   else if (prod==1){
    c->SaveAs(Form("~/www/HighMassStudy/16.7.19/eff_VBF_all_%s.png",lepName[j]));
    c->SaveAs(Form("~/www/HighMassStudy/16.7.19/eff_VBF_all_%s.pdf",lepName[j]));}
 }
 f.Close();
}