...
int col[2][3]={{2,3,4},{634,418,602}};
 ...
 //write to file
   ...
   c->Update();
   if (prod==0){ 
    c->SaveAs(Form("~/www/HighMass/160706/eff_ggH_all_%s.png",lepName[j]));
    c->SaveAs(Form("~/www/HighMass/160706/eff_ggH_all_%s.pdf",lepName[j]));}
   else if (prod==1){
    c->SaveAs(Form("~/www/HighMass/160706/eff_VBF_all_%s.png",lepName[j]));
    c->SaveAs(Form("~/www/HighMass/160706/eff_VBF_all_%s.pdf",lepName[j]));}
 }
 f.Close();
}
