...
//get the right directory automatically
  ...
  int inputfiles_ggH[]={1000,400,450,500,550,600,700,750,800,900};
  ...
  vector<TString> files_ggH;
   ...
   for (int i=0; i<Nfiles_ggH; i++) {
	 sprintf(inputfile,"2016_VBF_2l2q/mytree/PT13TeV/VBF_zz2l2q_M%d/ZZ2l2qAnalysis.root",inputfiles_ggH[i]);
...
// bin contents of merged histograms
 double M[]={1000,400,450,500,550,600,700,750,800,900};
...
//auto resolve directory name
   ...
   if(pchar != 0)
   strcpy(pchar, "/r/rbarr/www/2l2q_VBF/\0");

   pchar = strstr(dest, "/c/cayou");
   if(pchar != 0)
   strcpy(pchar, "/c/cayou/www/HighMass/\0");
   
   sprintf(dest+strlen(dest), "vbf_efficiency_%smerged_exclude_%d", (local_ZZCandType==2)?"un":"", exclude);
...
void ploteff_sig_spin2_VBF() {
	for(local_ZZCandType=1; local_ZZCandType<3;++local_ZZCandType) {
		for (exclude=0;exclude<3;exclude+=1) {
